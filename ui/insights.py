...
# Severity tags for insights (canonical lowercase strings used for grading).
# These mirror :class:`ui.status.InsightSeverity` values so that structured
# insight objects and legacy severity strings remain interchangeable.
...
def _severity_to_str(severity) -> str:
    """Normalize a severity value to its canonical lowercase string.

    Handles plain strings (``"critical"``) and enum members that subclass
    ``str`` (e.g. :class:`ui.status.InsightSeverity`), whose ``str()``
    representation is the enum name rather than the underlying value.
    """
    if isinstance(severity, str) and not isinstance(severity, enum.Enum):
        return severity
...
def _coerce_severities(insights, severities):
    """Return a list of canonical severity strings for grading.

    Priority:
      1. An explicit ``severities`` list (parallel to ``insights``).
      2. The ``.severity`` attribute carried by structured insight objects
         (e.g. :class:`ui.status.PerformanceInsight`).
      3. ``"neutral"`` as a safe fallback for plain strings without metadata.
    """
    if severities is not None:
        return [_severity_to_str(s) for s in severities]
    resolved = []
    for item in insights:
        sev = getattr(item, "severity", None)
        if sev is not None:
            resolved.append(_severity_to_str(sev))
        else:
            resolved.append(SEVERITY_NEUTRAL)
    return resolved
...
def _with_severities(insights):
    """Return ``(insights, severities)`` where ``severities`` is a parallel
    list of canonical severity strings derived from each insight's
    ``.severity`` attribute (falling back to ``"neutral"``).

    This preserves the structured :class:`PerformanceInsight` objects while
...
def generate_performance_insights(df, test_type, model_id=""):
    ...
    Returns:
        Tuple of (insights, severities) — ``insights`` is a list of
        structured :class:`ui.status.PerformanceInsight` objects (markdown
    ...
    if quality_warnings:
        ...
        has_critical = any(
            warning.severity is InsightSeverity.CRITICAL for warning in quality_warnings
        )
        if has_critical:
            insights.append(
                _insight(
            ...
            return _with_severities(insights)
...
def get_performance_grade(insights, severities=None):
    ...
    ``insights`` may be either:

    * Structured :class:`ui.status.PerformanceInsight` objects (whose
      ``.severity`` attribute is read directly), or
    * Legacy plain strings, optionally accompanied by a parallel
      ``severities`` list of canonical severity strings ("positive",
    ...
    # Categorize each insight by severity.
    # severities may be passed explicitly (parallel list, legacy callers) or
    # derived from each insight's ``.severity`` attribute (structured insights).
    resolved_severities = _coerce_severities(insights, severities)
    positive_count = resolved_severities.count(SEVERITY_POSITIVE)
    neutral_count = resolved_severities.count(SEVERITY_NEUTRAL)
    warning_count = resolved_severities.count(SEVERITY_WARNING)
    critical_count = resolved_severities.count(SEVERITY_CRITICAL)
