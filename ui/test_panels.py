...
def _execute_pending_test(run_test_func, test_type):
    ...
    # 调试：显示状态
    st.write(
        f"[DEBUG] pending type: {pending.get('test_type')}, is_resuming from session: {st.session_state.get('is_resuming', False)}"
    )
    st.write(
        f"[DEBUG] resume_data exists: {'resume_data' in st.session_state}, value: {bool(st.session_state.get('resume_data'))}"
    )
