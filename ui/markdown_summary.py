def _generate_markdown_summary(df, test_type, duration):
    ...
    from core.result_metrics import count_successful_requests, success_mask_from_error
    ...
    if test_type == "concurrency":
        ...
        if len(conc_levels) >= 2:
            ...
            if len(ttft_vals) >= 3:
                ...
                if jump_pct > 10:
                    ...
                    md += "- **TTFT Trend**: increased steadily with concurrency.\n"
            ...
            if tp_col:
                ...
                if len(tp_vals) >= 3:
                    ...
                    if saturation_c:
                        ...
                        md += "- **Throughput Saturation**: not yet saturated, still has growth capacity.\n"

    elif test_type == "prefill":
        levels = (
            sorted(df_ok["input_tokens_target"].unique())
            if "input_tokens_target" in df_ok.columns
            else []
        )
        if len(levels) >= 2:
            ...
            if len(ps_vals) >= 3:
                ...
                if drop_pct > 10:
                    ...
                    md += "- **Speed Degradation Pattern**: decreased steadily with increasing input length.\n"
        ...
        levels = (
            sorted(df_ok["context_length_target"].unique())
            if "context_length_target" in df_ok.columns
            else []
        )
        if len(levels) >= 2:
            ...
            if len(ttft_vals) >= 3:
                ...
                if jump_pct > 20:
                    ...
                    md += "- **TTFT Trend**: grew uniformly with increasing context.\n"
        ...
        if len(levels) >= 2:
            ...
            if len(cache_vals) >= 3:
                ...
                if increasing and cache_vals[-1] > cache_vals[0] + 5:
                    ...
                    md += "- **Cache Rate Trend**: shows fluctuation, cache effectiveness is unstable.\n"
                else:
                    md += "- **Cache Rate Trend**: overall stable.\n"
