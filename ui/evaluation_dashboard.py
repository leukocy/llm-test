        ...
        fig = go.Figure(
            data=[
                go.Bar(
                    ...
                    marker_color=[
                        "#ef4444" if v > 0.3 else "#eab308" if v > 0.1 else "#22c55e"
...
def render_sample_details_table(samples: list[dict[str, Any]], max_display: int = 20):
    ...
    with col1:
        filter_option = st.selectbox(
            "Filter", ["All", "Correct Only", "Errors Only"], key="sample_filter"
        )
    with col2:
        sort_option = st.selectbox(
            "Sort", ["Default", "By Latency", "By Reasoning Quality"], key="sample_sort"
        )
