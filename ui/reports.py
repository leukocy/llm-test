...
json.JSONEncoder.default = _patched_json_default  # type: ignore[method-assign]
...
def _render_test_summary_card(
    model_id, provider, duration, test_config, system_info=None, df=None, test_type=None
):
    """
        ...
        if df is not None and not df.empty:
                ...
                if len(conc_levels) >= 2:
                    ...
                    with m2:
                        st.metric(
                            f"TTFT @ C{int(lo)}",
                            f"{ttft_lo:.3f}s",
                            help="Low-concurrency baseline latency",
                        )
                    with m3:
                    ...
                    with m4:
                        st.metric(
                            ...
                            f"{peak_tp:.1f} t/s",
                            delta=f"@ C{peak_c}" if tp_col else None,
                        )
                    ...
                    if len(ttft_vals) >= 3:
                        ...
                        jump_pct = (
                            (diffs[max_jump_idx] / ttft_vals[max_jump_idx] * 100)
                            if ttft_vals[max_jump_idx] > 0
                            else 0
                        )
                        ttft_trend = (
                            f"C{jump_from}→C{jump_to} jumped {jump_pct:.0f}%"
                            if jump_pct > 10
                    ...
                    if tp_col:
                        ...
                        if len(tp_vals) >= 3:
                            ...
                            for i in range(1, len(tp_vals)):
                                gain = (
                                    (tp_vals[i] - tp_vals[i - 1]) / tp_vals[i - 1]
                                    if tp_vals[i - 1] > 0
                                    else 0
                                )
                    ...
                    with m11:
                        st.metric(
                            "TTFT Trend",
                            ttft_trend,
                            help="TTFT change pattern as concurrency increases",
                        )
                    with m12:
                        st.metric(
                            "Throughput Saturation",
                            saturation_label,
                            help="Inflection point where throughput growth drops below 5%",
                        )
                else:
                ...
                if len(levels) >= 2:
                    ...
                    with m6:
                        ...
                        st.metric(
                            f"TTFT @ {_fmt(lg)}t",
                            f"{ttft_lg:.3f}s",
                            delta=f"↑ {g:.1f}x" if g > 1.5 else "Stable",
                            delta_color="inverse",
                        )
                    with m7:
                        st.metric("Total Input Tokens", _fmt(total_input))
                    ...
                    if len(ps_vals) >= 3:
                        ...
                        speed_trend = (
                            f"{_fmt(drop_from)}→{_fmt(drop_to)} dropped {drop_pct:.0f}%"
                            if drop_pct > 10
                    ...
                    if len(levels) >= 3:
                        ...
                        ttft_ratio_full = (
                            ttft_by_level[levels[-1]] / ttft_by_level[levels[0]]
                            if ttft_by_level[levels[0]] > 0
                            else 1
                        )
                    ...
                    with m9:
                        st.metric(
                            "Peak Prefill Speed",
                            f"{peak_ps_val:.0f} t/s",
                            delta=f"@ {_fmt(peak_ps_level)}t",
                        )
                    with m10:
                        st.metric(
                            "Best TTFT",
                            f"{best_ttft_val:.3f}s",
                            delta=f"@ {_fmt(best_ttft_level)}t",
                        )
                    with m11:
                        st.metric(
                            "Speed Decline Point",
                            speed_trend,
                            help="Location of the largest single drop in Prefill speed",
                        )
                    with m12:
                        st.metric(
                            "TTFT Growth Pattern",
                            ttft_scaling,
                            help="TTFT growth relationship relative to input length",
                        )
                else:
                    _render_generic_summary(
                        st,
            ...
            # ===== LONG CONTEXT =====
                ...
                if len(levels) >= 2:
                    ...
                    sh_df, lo_df = (
                        df_ok[df_ok["context_length_target"] == sh],
                        df_ok[df_ok["context_length_target"] == lo],
                    )
                    ...
                    with m3:
                        st.metric(
                            f"TTFT @ {_fmt(lo)}",
                            f"{ttft_lo2:.3f}s",
                            delta=f"↑ {growth:.1f}x" if growth > 1.5 else "Stable",
                            delta_color="inverse",
                        )
                    with m4:
                    ...
                    if len(ttft_vals) >= 3:
                        jumps = [
                            (
                                (ttft_vals[i + 1] - ttft_vals[i]) / ttft_vals[i] * 100
                                if ttft_vals[i] > 0
                                else 0
                            )
                        ...
                        ttft_inflection = (
                            f"{_fmt(jump_from)}→{_fmt(jump_to)} jumped {jump_pct:.0f}%"
                            if jump_pct > 20
                            else "Uniform growth"
                        )
                    else:
                    ...
                    with m9:
                        st.metric(
                            "Peak Prefill Speed",
                            f"{peak_ps_val:.0f} t/s",
                            delta=f"@ {_fmt(peak_ps_ctx)} ctx",
                        )
                    with m10:
                        st.metric(
                            "Peak TPS",
                            f"{peak_tps_val:.1f} t/s",
                            delta=f"@ {_fmt(peak_tps_ctx)} ctx",
                        )
                    with m11:
                        st.metric(
                            "TTFT Inflection",
                            ttft_inflection,
                            help="Context length segment with the largest single TTFT increase",
                        )
                    with m12:
                        st.metric(
                            "TPS Volatility",
                            tps_stability,
                            help="Coefficient of variation of TPS across context lengths",
                        )
                else:
                    _render_generic_summary(
                        st,
            ...
            # ===== SEGMENTED (Prefix Caching) =====
                ...
                if len(levels) >= 2:
                    ...
                    f_df, l_df = (
                        df_ok[df_ok["context_length_target"] == f_seg],
                    ...
                    with m2:
                        st.metric(
                            ...
                            f"{cache_rate:.1f}%",
                            help=f"Hit {_fmt(cache_total)} / Total {_fmt(total_input)}",
                        )
                    with m3:
                    ...
                    for lv in levels:
                        ...
                        ch = (
                            int(lv_df["cache_hit_tokens"].sum())
                    ...
                    if len(cache_vals) >= 3:
                        ...
                        cache_trend = (
                            ...
                            else ("Volatile" if not increasing else "Stable")
                        )
                    else:
                    ...
                    with m9:
                        st.metric(
                            "Peak Cache Rate",
                            f"{peak_cache_rate:.0f}%",
                            delta=f"@ {_fmt(peak_cache_seg)} ctx",
                        )
                    with m10:
                        st.metric(
                            "Min Uncached TTFT",
                            f"{best_ttft_val:.3f}s",
                            delta=f"@ {_fmt(best_ttft_seg)} ctx",
                        )
                    with m11:
                        st.metric(
                            "Peak Prefill Speed",
                            f"{peak_ps_val:.0f} t/s",
                            delta=f"@ {_fmt(peak_ps_seg)} ctx",
                        )
                    with m12:
                        st.metric(
                            "Cache Rate Trend",
                            cache_trend,
                            help="Overall trend of cache hit rates across segments",
                        )
                else:
                    _render_generic_summary(
                        st,
            ...
            # ===== MATRIX =====
                ...
                test_type == "matrix"
                and "concurrency" in df.columns
                and "context_length_target" in df.columns
            ):
                tp_col = (
                    "system_output_throughput"
                    if "system_output_throughput" in df_ok.columns
                ...
                with m2:
                    st.metric(
                        "Test Matrix",
                        f"{len(conc_levels)}×{len(ctx_levels)}",
                        help=f"Concurrency: {conc_levels}\nContext: {ctx_levels}",
                    )
                if tp_col:
                    combo = (
                    ...
                    with m3:
                        st.metric(
                            "Max Throughput",
                            f"{best[tp_col]:.1f} t/s",
                            delta=f"C{int(best['concurrency'])}×{_fmt(best['context_length_target'])}ctx",
                        )
                    with m4:
                        st.metric(
                            "Min Throughput",
                            f"{worst[tp_col]:.1f} t/s",
                            delta=f"C{int(worst['concurrency'])}×{_fmt(worst['context_length_target'])}ctx",
                        )
                else:
                    with m3:
                        st.metric("Total Input Tokens", _fmt(total_input))
...
def _render_generic_summary(
    st_mod, df, total_requests, successful, total_input, total_output, duration
):
    ...
    if total_input == 0 and total_output == 0 and ttft_data.empty:
        st_mod.warning(
            "⚠️ **Data Quality Alert**: TTFT, TPS, and Token counts are all 0 or missing. The test may not have correctly captured performance metrics. Please check the API response usage info and streaming output."
        )
    ...
    with m6:
        pf = (
            df[df["prefill_speed"] > 0]["prefill_speed"].mean()
            if "prefill_speed" in df.columns
            else 0
        )
...
def generate_concurrency_report(
        ...
        column_config={
            **{k: st.column_config.Column(help=v) for k, v in COLUMN_TOOLTIPS.items()},
            "Best_TTFT (s)": st.column_config.ProgressColumn(
                ...
                min_value=0,
                max_value=(
                    safe_positive_max(summary["Best_TTFT (s)"], 1.2)
    ...
    with col1:
        fig1 = plot_plotly_line(
            ...
            "concurrency_str",
            "Best_TTFT (s)",
            f"Time To First Token (TTFT) {io_label}",
            "Concurrency",
            "Time (s)",
            model_id,
            rounds_per_level,
    ...
    with col3:
        fig_input = plot_plotly_line(
            summary,
            "concurrency_str",
            "Max_System_Input_Throughput (tokens/s)",
...
def generate_long_context_report(
    ...
    try:
        ...
        if "missing 'context_length_target'" in message:
            st.error(
                "Long context test report failed: missing 'context_length_target' column in data."
            )
        else:
    ...
    styled_table = create_styled_summary_table(
        ...
        highlight_cols=[
            ...
            for c in ["Best_TTFT", "Max_Prefill_Speed", "Max_System_Output_Throughput"]
        ],
        highlight_best=True,
    )
    st.dataframe(
    ...
    with col4:
        fig2 = plot_plotly_line(
            ...
            "x_label",
            "Max_System_Output_Throughput (tokens/s)",
            f"Sys Output Throughput {out_label}",
            "Context Length",
            "Tokens/s",
            model_id,
            requests_per_level,
    ...
    fig4 = plot_plotly_line(
        summary,
        "x_label",
        "Max_System_Throughput (tokens/s)",
        f"Sys Total Throughput {out_label}",
        "Context Length",
        "Tokens/s",
        model_id,
        requests_per_level,
...
def generate_matrix_report(
    ...
    stats = (
        group_copy.groupby(["context_length_target", "concurrency"])
        .agg(
        ...
        column_config={
            **{k: st.column_config.Column(help=v) for k, v in COLUMN_TOOLTIPS.items()},
            "Best_TTFT (s)": st.column_config.ProgressColumn(
                ...
                min_value=0,
                max_value=(
                    safe_positive_max(summary_display["Best_TTFT (s)"], 1.2)
...
def generate_segmented_report(
    ...
    if "context_length_target" not in df_group.columns:
        st.error(
            "Segmented context test report failed: missing 'context_length_target' column in data."
        )
        return ""
    group_copy = df_group.copy()
    if group_copy.empty:
    ...
    styled_table = create_styled_summary_table(
        ...
        highlight_cols=[
            seg_rename_map.get(c, c)
            for c in ["Uncached_TTFT", "Max_Prefill_Speed", "Cache_Hit_Rate"]
        ],
        highlight_best=True,
    )
        ...
        column_config={
            **{k: st.column_config.Column(help=v) for k, v in seg_tooltips.items()},
            "Uncached_TTFT (s)": st.column_config.ProgressColumn(
                ...
                min_value=0,
                max_value=(
                    safe_positive_max(stats["Uncached_TTFT (s)"], 1.2)
            ...
            "Max_Prefill_Speed (tokens/s)": st.column_config.ProgressColumn(
                ...
                min_value=0,
                max_value=(
                    safe_positive_max(stats["Max_Prefill_Speed (tokens/s)"], 1.1, 1000)
    ...
    if (
        ...
        insights.append(
            f"🏆 **Best Uncached TTFT**: `{best_ttft_row['Uncached_TTFT (s)']:.4f}s` (segment: {best_ttft_row['context_length_target']})"
        )
    ...
    with col1:
        fig1 = plot_plotly_line(
            ...
            "x_label",
            "Uncached_TTFT (s)",
            "Time To First Token (Uncached) by Segment",
            "Context Segment",
            "Time (s)",
            model_id,
            requests_per_level,
