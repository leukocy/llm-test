...
def render_engine_runtime_panel(api_base_url: str) -> None:
    ...
    with st.sidebar.expander("推理引擎接入（记录引擎运行）", expanded=False):
        ...
        metrics_url = st.text_input(
            "引擎 metrics 端点",
            value=er.get("metrics_url", "") or default_url,
            help="如 http://gpu-host:8000/metrics",
            key="er_metrics_url",
        )
        log_path = st.text_input(
            "引擎启动日志路径（可选）",
            value=er.get("log_path", ""),
        ...
        # 自适应测试：KV 预算（手动覆盖 > 自动探测）。留空则测试前自动从 /metrics 或
        # /v1/models 探测；给了则用此值，超预算的 (并发×上下文×输出) cell 自动跳过。
        kv_budget = st.number_input(
            "KV 预算（tokens，自适应跳过）",
            min_value=0,
            value=int(er.get("kv_budget") or 0),
            step=4096,
            key="er_kv_budget",
            help="留空/0 = 测试前自动从引擎 /metrics 探测 KV 容量；填了则用手动值。"
            "超预算的 cell（conc×(ctx+max_tokens) > 预算）自动跳过，error 列记 over_kv_budget。",
        )
        ...
        poll_interval = st.number_input(
            ...
            min_value=1,
            max_value=300,
            value=int(er.get("poll_interval", 5)),
            step=1,
            key="er_poll_interval",
            help="引擎 /metrics 轮询间隔（秒）。越大引擎日志越安静，但采样点越稀疏。",
        )

        st.session_state.engine_runtime = {
            "metrics_url": (metrics_url or "").strip() or None,
            "log_path": (log_path or "").strip() or None,
            "enabled": enabled,
            "kv_budget": kv_budget or None,
    ...
    with st.sidebar.expander("模型架构规格（等效带宽）", expanded=False):
        ...
        with c1:
            total = st.number_input(
                "总参数 (B)",
                min_value=0.0,
                value=float(spec.total_params_b or 0) if spec else 0.0,
                step=1.0,
                key="ms_total_params",
                help="total_params_b",
            )
            experts = st.number_input(
                "专家数",
                min_value=0,
                value=int(spec.num_experts or 0) if spec else 0,
                step=1,
                key="ms_num_experts",
            )
            ctx = st.number_input(
                "上下文上限",
                min_value=0,
                value=int(spec.max_position_embeddings or 0) if spec else 0,
                step=1024,
                key="ms_max_context",
        ...
        with c2:
            active = st.number_input(
                "激活参数 (B)",
                min_value=0.0,
                value=float(spec.active_params_b or 0) if spec else 0.0,
                step=1.0,
                key="ms_active_params",
                help="active_params_b (MoE 每次激活)",
            )
            topk = st.number_input(
                "top-k 专家",
                min_value=0,
                value=int(spec.num_experts_per_tok or 0) if spec else 0,
                step=1,
                key="ms_top_k",
            )
            dtype = st.selectbox(
        ...
        with cc1:
            kv_dtype = st.selectbox(
                ...
                ["", "fp16", "fp8", "int4"],
                index=(
                    ["", "fp16", "fp8", "int4"].index(spec.kv_dtype)
                    if spec and spec.kv_dtype in ["fp16", "fp8", "int4"]
                    else 0
                ),
                key="ms_kv_dtype",
                help="KV cache 精度（可与权重精度不同）",
            )
        with cc2:
            multimodal = st.checkbox(
                "多模态",
                value=bool(spec.is_multimodal) if spec else False,
                key="ms_multimodal",
            )

        mtp = st.checkbox(
            "支持 MTP / 推测解码",
            value=bool(spec.supports_mtp) if spec else False,
            key="ms_supports_mtp",
            help="Multi-Token Prediction / 投机解码",
        )
...
def render_serving_config_panel() -> None:
    ...
    with st.sidebar.expander("服务/启动配置（引擎/并行/后端）", expanded=False):
        ...
        engine = st.selectbox(
            ...
            [""] + ENGINES,
            index=(
                ([""] + ENGINES).index(sc.get("engine", ""))
                if sc.get("engine", "") in ENGINES
                else 0
            ),
            key="sc_engine",
        )
        engine_version = st.text_input(
            "引擎确切版本", value=sc.get("engine_version", ""), key="sc_engine_version"
        )
        ...
        attn = st.selectbox(
            ...
            [""] + ATTENTION_BACKENDS,
            index=(
                ([""] + ATTENTION_BACKENDS).index(sc.get("attention_backend", ""))
                if sc.get("attention_backend", "") in ATTENTION_BACKENDS
                else 0
            ),
            key="sc_attn",
            help="如 vLLM VLLM_ATTENTION_BACKEND",
        )
        moe = st.selectbox(
            "MoE 后端",
            [""] + MOE_BACKENDS,
            index=(
                ([""] + MOE_BACKENDS).index(sc.get("moe_backend", ""))
                if sc.get("moe_backend", "") in MOE_BACKENDS
                else 0
            ),
            key="sc_moe",
        )
        kv_dtype = st.selectbox(
            ...
            ["", "auto", "fp16", "fp8"],
            index=(
                ["", "auto", "fp16", "fp8"].index(sc.get("kv_cache_dtype", ""))
                if sc.get("kv_cache_dtype", "") in ["auto", "fp16", "fp8"]
                else 0
            ),
            key="sc_kv_dtype",
        )
        ...
        with cc1:
            max_len = st.number_input(
                "max_model_len",
                min_value=0,
                value=int(sc.get("max_model_len") or 0),
                step=4096,
                key="sc_max_len",
            )
        with cc2:
            gpu_util = st.number_input(
                ...
                max_value=1.0,
                step=0.05,
                value=float(sc.get("gpu_memory_utilization") or 0.0),
                key="sc_gpu_util",
            )

        chunked = st.checkbox(
            "chunked prefill", value=bool(sc.get("chunked_prefill")), key="sc_chunked"
        )
        prefix = st.checkbox(
            "prefix caching", value=bool(sc.get("prefix_caching")), key="sc_prefix"
        )
        mtp_on = st.checkbox(
            "开启 MTP / 推测解码", value=bool(sc.get("mtp_enabled")), key="sc_mtp_on"
        )
        if mtp_on:
            spec_method = st.text_input(
                "推测方法",
                value=sc.get("speculative_method", ""),
                placeholder="EAGLE3 / ngram / MTP",
                key="sc_spec_method",
            )
            num_spec = st.number_input(
                "num_speculative_tokens",
                min_value=0,
                value=int(sc.get("num_speculative_tokens") or 0),
                step=1,
                key="sc_num_spec",
            )
        else:
        ...
        st.session_state.serving_config = {
            ...
            "gpu_memory_utilization": gpu_util or None,
...
def render_test_metadata_panel() -> None:
    ...
    with st.sidebar.expander("测试元数据 & 可对外等级", expanded=False):
        ...
        next_action = st.text_input(
            "下一步动作 (next_action)", value=tm.get("next_action", ""), key="tm_next"
        )
        comparison_group = st.text_input(
            "对照组 (comparison_group)",
            value=tm.get("comparison_group", ""),
            help="如 mtp_on / mtp_off 用于对比",
            key="tm_group",
        )
        supersedes = st.text_input(
            "复测指向 (supersedes_test_id)",
