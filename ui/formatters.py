...
COLUMN_DISPLAY_NAMES = {
    ...
    "tps": "TPS (t/s)",
    "tpot": "TPOT (s)",
    "tpot_p95": "TPOT Chunk P95 (s)",
    "tpot_p99": "TPOT Chunk P99 (s)",
    "rps": "RPS (req/s)",
...
FORMAT_RULES = {
    "round_4": {"TTFT (s)", "TPOT (s)", "TPOT Chunk P95 (s)", "TPOT Chunk P99 (s)"},
    "round_1": {
        "Prefill Speed (t/s)",
        "TPS (t/s)",
        "RPS (req/s)",
        "System Throughput (t/s)",
        "Input Throughput (t/s)",
    ...
    "int": {
        "Prefill Tokens",
        "Decode Tokens",
        "Prefill Tokens (Effective)",
        "Decode Tokens (Effective)",
        "Cache Hit (tokens)",
...
def format_results_for_display(
    ...
    ordered = TEST_TYPE_DISPLAY_ORDER.get(test_type or "", [])

    # Handle column fallback (segmented test effective_* columns)
    fallback = (
        SEGMENTED_COLUMN_FALLBACK
        if test_type in ("Segmented Context Test", "分段onunder文Test")
        else {}
    )
    ...
    # Invalid zero values in latency/throughput metrics should not be shown as real data.
