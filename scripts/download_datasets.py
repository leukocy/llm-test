...
def download_gpqa(output_dir="datasets/gpqa", force=False):
    ...
    try:
        ...
        for item in ds:
            samples.append({
                "question": item.get("Question", ""),
                "choices": [item.get(k, "") for k in ("Correct Answer", "Incorrect Answer 1",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gpqa", action="store_true", help="under载 GPQA Diamond (Science题型)")
    parser.add_argument("--mbpp", action="store_true", help="under载 MBPP (Code题型)")
    parser.add_argument("--longbench", action="store_true", help="under载 LongBench (Longform题型)")
    parser.add_argument("--swebench", action="store_true", help="under载 SWE-Bench Lite (Code题型)")
