...
ENDPOINTS = {
    name: url
    for name, url in [("Server1", os.getenv("SERVER1_URL")), ("Server2", os.getenv("SERVER2_URL"))]
    if url
}
    ...
    with requests.post(f"{base_url}/chat/completions", json=payload,
        ...
        for line in r.iter_lines():
            ...
            if l.startswith('data: '):
                ...
                try:
                    c = json.loads(d)
                    if c.get("usage"): usage_info = c["usage"]
...
for name, base_url in ENDPOINTS.items():
    ...
    try:
        ...
        if not model_id: log("noModel"); continue
        log(f"Model: {model_id}")
    except Exception as e:
        log(f"Connection failed: {e}"); continue
    ...
    for i in range(1, 5):
        ...
        if result["usage"]:
            ...
            for key in ["cache_hit_tokens", "prompt_cache_hit_tokens"]:
                if result["usage"].get(key): cache_hit = result["usage"][key]
        ...
        log(f"  #{i}: TTFT={ttft:.4f}s | 总耗时={total:.2f}s | prompt={prompt_tokens} | cache_hit_usage={cache_hit}{marker}")
        time.sleep(2)
