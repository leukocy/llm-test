...
async def test_all():
    ...
    try:
        ...
        if not results.empty:
            ...
            for _, r in results.iterrows():
                print(
                    f"  Concurrency {int(r['concurrency'])}: TTFT={r['ttft']:.4f}s, "
    ...
    try:
        results = await runner.run_prefill_test(
            token_levels=[256, 512, 1024], requests_per_level=1, max_tokens=50
        )
        ...
        if not results.empty:
            ...
            for _, r in results.iterrows():
                print(
                    f"  {int(r['context_length_target'])} tokens: TTFT={r['ttft']:.4f}s, "
    ...
    try:
        results = await runner.run_long_context_test(
            context_lengths=[1024, 2048, 4096], rounds_per_level=1, max_tokens=50
        )
    ...
    try:
        results = await runner.run_throughput_matrix_test(
            concurrencies=[1, 2], context_lengths=[256, 512], rounds=1, max_tokens=50
        )
        ...
        if not results.empty:
            ...
            for _, r in results.iterrows():
                print(
                    f"  Concurrency={int(r['concurrency'])}, Context={int(r['context_length_target'])}: "
    ...
    try:
        results = await runner.run_stability_test(
            concurrency=1, duration_seconds=10, max_tokens=50, context_length_target=256
        )
