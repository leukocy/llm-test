...
class TestTestExecutor:
    ...
    @patch("core.benchmark_runner.BenchmarkRunner")
    def test_capture_system_info_with_custom_overrides(
        self, mock_runner_class, executor, clean_session_state
    ):
    ...
    def test_capture_system_info_fallbacks(
        ...
        # engine_name has no automatic fallback — it stays 'Unknown' unless user provides it
        assert sys_info["engine_name"] == "Unknown"
    ...
    def test_calculate_total_requests_prefill_test(self, executor):
        ...
        executor._calculate_total_requests(
            dummy_func, mock_runner, (token_levels, requests_per_level, 500)
        )
    ...
    def test_capture_test_config_concurrency(
        self, executor, clean_session_state, session_state_module
    ):
        ...
        executor._capture_test_config(
            test_function, "并发性能Test", ([1, 2], 3, 100, 2000), "20250130_120000"
        )
    ...
    def test_capture_test_config_long_context(
        self, executor, clean_session_state, session_state_module
    ):
        ...
        executor._capture_test_config(
            test_function, "长onunder文Test", ([2000, 4000], 5, 500), "20250130_120000"
        )
    ...
    def test_execute_test_async_shuts_down_loop_resources(self, executor, monkeypatch):
        """Test async runner closes async generators before closing the loop."""
        import asyncio
        ...
        async def shutdown_asyncgens():
            shutdown_calls["asyncgens"] = True

        async def shutdown_default_executor():
            shutdown_calls["executor"] = True
        ...
        async def test_function(_runner):
            return pd.DataFrame()

        result = executor._execute_test_async(test_function, object(), ())
