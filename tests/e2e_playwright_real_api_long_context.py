"""
Playwright E2E test for Long Context Test against the real 4-card GPU API.

Uses a single short context length (1024) and 1 round / 1 max token
to keep the test fast while still verifying end-to-end API connectivity.
...
Run: python tests/e2e_playwright_real_api_long_context.py
"""
...
def start_streamlit():
    ...
    for i in range(STREAMLIT_TIMEOUT * 2):
        try:
            ...
            print("Streamlit ready on http://localhost:8503")
            return
        except Exception:
            time.sleep(0.5)
...
def test_real_api_flow(page):
    ...
    take_screenshot(page, "lc_01_homepage")
    ...
    time.sleep(0.3)
    take_screenshot(page, "lc_02_url_filled")
    ...
    time.sleep(0.3)
    take_screenshot(page, "lc_03_model_set")

    print("\n=== Step 4: Switch to Long Context Test ===")
    idx = find_test_type_selectbox(page)
    if idx is None:
        print("FAIL: No selectbox available")
        return False
    select_by_typing(page, idx, "Long Context")
    time.sleep(0.6)
    take_screenshot(page, "lc_04_long_context")
    ...
    page.keyboard.press("Escape")
    time.sleep(0.2)
    print("PASS: Context length set to 1024 only")
    take_screenshot(page, "lc_05_context_set")

    print("\n=== Step 6: Reduce Max Output Tokens to 1 ===")
    ...
    take_screenshot(page, "lc_06_params_set")

    print("\n=== Step 7: Click Start Long Context Test ===")
    page.evaluate("window.scrollTo(0, 300)")
    time.sleep(0.2)

    start_btn = page.locator("button").filter(has_text="Start Long Context Test")
    ...
    take_screenshot(page, "lc_07_clicked")
    ...
    take_screenshot(page, "lc_08_running")
    ...
    print("PASS: Long Context Test finished without visible errors")
    return True


def main():
    print("=" * 60)
    print("Playwright E2E Long Context Test: Real 4-Card API")
