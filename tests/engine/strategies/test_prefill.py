...
def test_calculate_total_requests(strategy):
    params = {"input_tokens_list": [10, 20], "rounds": 5}
...
async def test_execute(strategy, mock_event_bus):
    ...
    params = {"input_tokens_list": [10], "rounds": 1, "max_tokens": 5}
    ...
    with patch("engine.strategies.prefill.ConcurrencyEngine") as MockEngineCls:
        ...
        mock_engine.run_batch = AsyncMock(
            return_value=[TestResult(session_id=1, ttft=0.05, prefill_tokens=10)]
        )
