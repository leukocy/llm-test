...
def session_state_module():
    ...
    if 'config' in sys.modules and hasattr(sys.modules['config'], 'session_state'):
        delattr(sys.modules['config'], 'session_state')
...
class TestSetTestCancelled:
    """Test set_test_cancelled function"""

    def test_sets_cancelled_state_without_private_streamlit_state(self, mock_session_state, session_state_module):
        """Test cancellation state does not rely on Streamlit private _state."""
        session_state_module.set_test_cancelled()

        assert mock_session_state._data['test_running'] == False
        assert mock_session_state._data['test_paused'] == False
        assert mock_session_state._data['test_status'] == "Cancelled"
        assert mock_session_state._data['stop_requested'] == False
        assert mock_session_state._data['pause_requested'] == False
        assert '_test_running_timestamp' in mock_session_state._data
