...
class RequestsSessionAsyncStreamClient:
    """Adapter that lets legacy requests-style fixtures exercise async providers."""

    def __init__(self, session):
        self.session = session

    def stream(self, method, url, *, json=None, headers=None, timeout=None):
        post_context = self.session.post(
            url,
            json=json,
            headers=headers,
            timeout=timeout,
        ...
        return _AsyncStreamContext(post_context)


class _AsyncStreamContext:
    def __init__(self, post_context):
        self.post_context = post_context
        self.response = None

    async def __aenter__(self):
        if hasattr(self.post_context, "__enter__"):
            self.response = self.post_context.__enter__()
        else:
            self.response = self.post_context
        return _AsyncStreamResponse(self.response)

    async def __aexit__(self, exc_type, exc, traceback):
        if hasattr(self.post_context, "__exit__"):
            self.post_context.__exit__(exc_type, exc, traceback)
        return False
...
class _AsyncStreamResponse:
    ...
    async def aread(self):
        text = getattr(self.response, "text", "")
...
def openai_client_from_session(session):
    return RequestsSessionAsyncStreamClient(session)
...
class TestRequestTimeout:
    def test_timeout_increases_for_ultra_long_contexts(self):
        assert get_request_timeout_seconds(input_tokens=127_999) == 600.0
        assert get_request_timeout_seconds(input_tokens=128_000) == 1200.0
        assert get_request_timeout_seconds(input_tokens=255_999) == 1200.0
        assert get_request_timeout_seconds(input_tokens=256_000) == 1200.0
        assert get_request_timeout_seconds(input_tokens=256_001) == 1800.0
        assert get_request_timeout_seconds(input_tokens=260_000) == 1800.0
        assert get_request_timeout_seconds(input_tokens=520_000) == 3600.0
        assert get_request_timeout_seconds(input_tokens=1_000_000) == 7200.0
...
class TestEdgeCases:
    ...
    async def test_multiple_extra_body_params(self, mock_requests_session):
        ...
        # Unknown OpenAI-compatible platforms should not receive vendor-specific params.
        await provider.get_completion(
            mock_client,
            session_id=1,
            prompt="Test",
