...
class DatasetManager:
    ...
    def _download_longbench(
        ...
        try:
            ...
            if not all_samples:
                self._log("LongBench: no sub-tasks loaded successfully")
                return False
            ...
            if progress_callback:
                progress_callback(1.0, "LongBench download complete")

            self._log(f"LongBench: saved {len(all_samples)} total samples")
            return True

        except Exception as e:
            self._log(f"LongBench download failed: {e}")
            return False
