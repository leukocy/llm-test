...
class ExecLogRepository(BaseRepository[ExecLog]):
    ...
    def insert(self, log: ExecLog) -> int | None:
        """InsertLog"""
        data = log.to_dict()
    ...
    def find_by_level(
        self, level: str, run_id: int | None = None, limit: int = 100
    ) -> list[ExecLog]:
        """based on级别查找"""
        if run_id:
    ...
    def find_errors(self, run_id: int | None = None, limit: int = 100) -> list[ExecLog]:
        """查找Error Logs"""
