...
输入是 ui.insights.generate_performance_insights 的输出（emoji 前缀字符串列表）+
...
_BOTTLENECK_RULES: list[tuple[list[str], str]] = [
    # 数据质量问题（最高优先级，结论不可信）
    (["data anomaly", "throughput anomaly", "prefill speed anomaly",
      "ttft missing", "tps missing"], "data_quality"),
    # 算力 / prefill 瓶颈
    (["long-text compute bottleneck", "low prefill speed", "high ttft",
      "compute bottleneck"], "compute_prefill"),
    # 并发饱和 / 调度
    (["overload degradation", "low concurrency throughput efficiency",
      "queuing", "contention"], "concurrency_saturation"),
    # 延迟 / KV
...
_ERROR_KEYWORDS: list[tuple[list[str], str]] = [
    ...
    (["overloaded", "503", "service unavailable", "502", "504", "connection"], "server"),
