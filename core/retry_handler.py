...
class RetryHandler:
    ...
    def _calculate_delay(self, attempt: int, retry_after: float | None = None) -> float:
        ...
        # Add jitter only for locally computed backoff. Retry-After is a server
        # minimum and should not be reduced by jitter.
        if self.config.jitter and retry_after is None:
...
async def retry_async(func: Callable[..., Awaitable[T]], *args, **kwargs) -> T:
    """
    便捷函数：带重试执行Async函数
