...
from core.result_metrics import calculate_success_rate, success_mask_from_error
...
class CertificationReportGenerator:
    ...
    def generate(self, model_id: str, results_df: Any) -> CertificationResult:
        ...
        if not s1_df.empty:
            ...
            if not ov_res.empty:
                if success_mask_from_error(ov_res["error"]).all():
                    met.append(
                        "Stage 1 - Overflow (105%): Gracefully handled (Model processed or engine truncated)."
                    )
                else:
                    met.append(
                        "Stage 1 - Overflow (105%): Engine returned error (Acceptable if non-crashing)."
                    )
            ...
            if not niah_res.empty:
                ...
                if niah_pass_rate >= MQC_THRESHOLDS["niah_min_pass_rate"]:
                    met.append(
                        f"Stage 1 - NIAH: {int(niah_passed)}/{niah_total} tests passed ({niah_pass_rate * 100:.0f}%)."
                    )
                else:
                    status = "FAIL"
                    failed.append(
                        f"Stage 1 - NIAH: Only {int(niah_passed)}/{niah_total} passed ({niah_pass_rate * 100:.0f}% < {MQC_THRESHOLDS['niah_min_pass_rate'] * 100:.0f}% threshold)."
                    )
            ...
            if not ttfts.empty:
                ...
                met.append(
                    f"Stage 1 Performance: P50={metrics['s1_ttft_p50']:.0f}ms, P99={metrics['s1_ttft_p99']:.0f}ms."
                )

            # Cache Hit Ratio Analysis (Phase 2 improvement)
            boundary_df = s1_df[s1_df["test_type"].str.contains("boundary")]
            if (
                not boundary_df.empty
                and "cache_hit_tokens" in boundary_df.columns
                and "prefill_tokens" in boundary_df.columns
            ):
                ...
                boundary_df["cache_hit_ratio"] = boundary_df.apply(
                    lambda row: (
                        (row["cache_hit_tokens"] / row["prefill_tokens"] * 100)
                        if row.get("prefill_tokens", 0) > 0
                        else 0
                    ),
                    axis=1,
                )
                ...
                if avg_cache_ratio > 50:
                    met.append(
                        f"Stage 1 - Cache Efficiency: Excellent ({avg_cache_ratio:.1f}% average hit rate)."
                    )
                elif avg_cache_ratio > 20:
                    met.append(
                        f"Stage 1 - Cache Efficiency: Moderate ({avg_cache_ratio:.1f}% average hit rate)."
                    )
                elif avg_cache_ratio > 0:
                    met.append(
                        f"Stage 1 - Cache Efficiency: Low ({avg_cache_ratio:.1f}% average hit rate, consider enabling Prefix Caching)."
                    )
                ...
                if len(boundary_df) >= 2:
                    ...
                    if last_cache > first_cache:
                        ...
                        met.append(
                            f"Stage 1 - Cache Trend: Progressive caching working ({first_cache:.1f}% → {last_cache:.1f}%)."
                        )
        ...
        if not s2_df.empty:
            ...
            success_rate = calculate_success_rate(burst_df["error"])
            metrics["burst_success_rate"] = success_rate

            # Use configurable thresholds
            if success_rate >= MQC_THRESHOLDS["burst_success_rate_pass"]:
                met.append(
                    f"Stage 2 - Burst: {success_rate * 100:.1f}% Success rate across {len(burst_df)} short prompts."
                )
            elif success_rate >= MQC_THRESHOLDS["burst_success_rate_warn"]:
                met.append(
                    f"Stage 2 - Burst: {success_rate * 100:.2f}% Success rate (Pass with minor jitter)."
                )
            else:
                status = "FAIL"
                failed.append(
                    f"Stage 2 - Burst: Stability below {MQC_THRESHOLDS['burst_success_rate_warn'] * 100:.0f}% ({success_rate * 100:.2f}%)."
                )
            ...
            if "system_output_throughput" in burst_df.columns:
                ...
                if avg_throughput > 0:
                    ...
                    met.append(
                        f"Stage 2 - Burst Throughput: {avg_throughput:.1f} tokens/sec average."
                    )
            ...
            if not burst_ttfts.empty:
                ...
                met.append(
                    f"Stage 2 Burst Performance: P50={metrics['s2_burst_ttft_p50']:.0f}ms, P99={metrics['s2_burst_ttft_p99']:.0f}ms."
                )
            ...
            if (
                success_mask_from_error(long_tasks["error"]).all()
                and success_mask_from_error(short_tasks["error"]).all()
            ):
                met.append(
                    f"Stage 2 - Mixed Load: Handled {len(mixed_df)} mixed tasks (80/20 s/l)."
                )
            else:
                status = "FAIL"
                failed.append(
        ...
        if not s3_df.empty:
            for version in ["A", "B", "C"]:
                ...
                if not v_df.empty:
                    ...
                    elif not last_round_success:
                        status = "FAIL"
                        failed.append(
                            f"Stage 3 - Sessions {version}: Crashed at round {last_round['round']}."
                        )
                    else:
                        status = "FAIL"
                        failed.append(
                            f"Stage 3 - Sessions {version}: Lost context/secret key by round {last_round['round']}."
                        )
        ...
        cert_result = CertificationResult(
            ...
            metrics={
                "success_rate": calculate_success_rate(results_df["error"]),
                "total_requests": len(results_df),
                "model_id": model_id,
            },
        )
