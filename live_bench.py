...
# ──────────────────────────────────────────────────────────────────────────
_WORD_POOL = (
    "system network kernel memory buffer cache latency throughput token context "
    "vector matrix tensor attention expert router sparse dense quantization inference "
    "prefill decode batch concurrency latency bandwidth gigabyte flops utilization "
    "queue schedule shard tensor parallel pipeline datacenter consistency byzantine "
    "erasure replication consensus checkpoint gradient optimizer attention fusion "
    "墨子 兼爱 非攻 逻辑 三段论 诸子 先秦 因果 分布式 一致性 容错 存储 缓存 调度 "
    "alpha bravo charlie delta echo foxtrot golf hotel india juliet kilo lima mike "
    "november oscar papa quebec romeo sierra tango uniform victor whiskey xray yankee "
    "request response timeout retry backoff idempotent stream fragment aggregate "
).split()
