...
PHASES = [
    (
        "A_lowconc",
        [1, 2, 4],
        [64, 1024, 2048, 4096, 8192, 16384, 32768, 65536, 131072, 258000],
    ),
...
async def one_req(i, prompt):
    ...
    # 收敛点:ITL 降到 1.2× 稳态
    steady_itl = (
        sorted(itl[-50:])[len(itl[-50:]) // 2]
        if len(itl) >= 50
        else (sorted(itl)[len(itl) // 2] if itl else 1)
    )
    converge = 0
    ...
    return {
        ...
        "tps_0_100": w(0, 100),
        "tps_0_50": w(0, 50),
        "tps_100_500": w(100, 500),
        "tps_500_2000": w(500, 2000),
...
async def main():
    ...
    for tag, concs, ctxs in PHASES:
        ...
        if n != 8:
            print(f"[ABORT] GPU={n}≠8", flush=True)
            break
        for conc in concs:
            for ctx in ctxs:
                ...
                if actual_max < 100:
                    ...
                    continue
                print(
                    f"  conc={conc} ctx={ctx} max_tokens={actual_max}...",
                    flush=True,
                    end="",
                )
                ...
                if ok:
                    ...
                    print(
                        f" {len(ok)}/{conc} ok {elapsed:.0f}s | agg={agg} steady={steady} 前100={first100} 收敛={conv}",
                        flush=True,
                    )
                else:
                ...
                if all_rows:
                    pd.DataFrame(all_rows).to_csv(
                        "raw_data/decode_steady_full.csv",
                        index=False,
                        encoding="utf-8-sig",
                    )
        n2 = gpu_count()
