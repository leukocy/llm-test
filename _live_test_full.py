...
def make_runner(tag):
    return BenchmarkRunner(
        ...
        tokenizer_option="API (usage field)", csv_filename=f"raw_data/full_{tag}.csv",
...
# 风险递增:低并发全上下文 → 高并发低上下文 → 高并发中上下文 → 高并发260K(最险)
PHASES = [
    ("A_safe_lowconc", [1, 2, 4], [64, 1024, 2048, 4096, 8192, 16384, 32768, 65536, 131072, 260000]),
    ("B_safe_highconc_lowctx", [8, 16, 32], [64, 1024, 2048, 4096, 8192]),
    ("C_mid_highconc_middctx", [8, 16, 32], [16384, 32768, 65536, 131072]),
    ("D_risky_highconc_260k", [8, 16, 32], [260000]),
]
...
async def main():
    ...
    for tag, conc, ctx in PHASES:
        ...
        if n != 8:
            ...
            return
        r = make_runner(tag)
        await r.run_throughput_matrix_test(concurrencies=conc, context_lengths=ctx, rounds=3, max_tokens=512)
        n2 = gpu_count()
