...
try:
    ...
    from core.enhanced_parser import get_parser
...
class BaseEvaluator(ABC):
    ...
    def _init_template(self):
        ...
        try:
            from core.prompt_template import TemplateFactory
            ...
            if os.path.exists(yaml_path):
                self._prompt_template = TemplateFactory.from_yaml(
                    yaml_path, format=self.prompt_format
                )
                return
            ...
            try:
                self._prompt_template = TemplateFactory.get(
                    self.dataset_name, format=self.prompt_format
                )
            except ValueError:
                pass
    ...
    def get_sample_category(self, sample: dict[str, Any]) -> str:
        ...
        """
        return str(sample.get("category", sample.get("subject", "")))
    ...
    async def evaluate_single(
        ...
        Args:
            ...
            get_response_func: Async function to call the LLM. Accepts (prompt, messages).
        ...
        try:
            ...
            if not is_correct and judge_enabled and response.strip():
                print(
                    f"⚡ [Judge Triggered] Sample {sample_id} | Model Answer Preview: {response[:30]}..."
                )
...
def extract_choice_answer(
    response: str, choices: list[str] | None = None, use_enhanced: bool = True
) -> str:
    """
    ...
    # Try new centralized parser first
    if use_enhanced:
        try:
            from .answer_parser import MultiChoiceParser

            return MultiChoiceParser().parse(response, choices)
        except Exception:
            pass
    ...
    if all_matches:
        return str(all_matches[-1])  # Usually the last one is the final choice

    return ""


def extract_numeric_answer(response: str, use_enhanced: bool = True) -> str:
    """
    ...
    if use_enhanced:
        try:
            ...
            return MathAnswerParser().parse(response)
        except Exception:
            pass
    ...
    if use_enhanced and ENHANCED_PARSER_AVAILABLE:
        try:
            parser = get_parser()
            result = parser.parse(response, EnhancedAnswerType.NUMBER)
...
def extract_number_answer(response: str, use_enhanced: bool = True) -> str:
    """Backward-compatible alias for numeric answer extraction."""
    return extract_numeric_answer(response, use_enhanced=use_enhanced)
